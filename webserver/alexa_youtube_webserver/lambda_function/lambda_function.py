...
# TODO: Change it back
DEVELOPER_KEY = config.get("youtube_api_key", os.environ.get('DEVELOPER_KEY'))
if not DEVELOPER_KEY:
    raise Exception("Youtube developer key not specified in configuration or environmental variable")
YOUTUBE_API_SERVICE_NAME = 'youtube'
YOUTUBE_API_VERSION = 'v3'
youtube = build(YOUTUBE_API_SERVICE_NAME, YOUTUBE_API_VERSION, developerKey=DEVELOPER_KEY)
from alexa_youtube_webserver.lambda_function.strings import *
...
def on_intent(event):
    ...
    search_intents = ["SearchIntent", "PlayOneIntent", "PlaylistIntent", "SearchMyPlaylistsIntent",
                      "ShuffleMyPlaylistsIntent", "ChannelIntent", "ShuffleIntent", "ShufflePlaylistIntent",
                      "ShuffleChannelIntent", "PlayMyLatestVideoIntent"]
...
def playlist_search(query, sr, do_shuffle='0'):
    ...
    while 'nextPageToken' in data and len(videos) < 200:
        ...
        data = youtube.playlistItems().list(part='snippet', maxResults=50, playlistId=playlist_id,
...
def channel_search(query, sr, do_shuffle='0'):
    ...
    while 'nextPageToken' in data and len(videos) < 200:
        next_page_token = data['nextPageToken']
        data = youtube.search().list(part='snippet', maxResults=50, channelId=playlist_id,
                                     pageToken=next_page_token).execute()
        for item in data['items']:
            try:
...
def search(event):
    ...
    for i, id in enumerate(videos):
        if playlist_channel_video != strings['video'] and time() - startTime > 8:
            return build_response(build_cardless_speechlet_response(
                playlist_channel_video + " " + playlist_title + " " + strings['notworked'], None, False),
                sessionAttributes)
        playlist['v' + str(i)] = id
        if next_url is None:
            playlist['p'] = i
    ...
    if next_url == False:
        return build_response(
            build_short_speechlet_response('This skill is being throttled by YouTube, please try again later', True))
    ...
    card_title = "Youtube"
    return build_response(
        build_audio_or_video_response(card_title, speech_output, should_end_session, next_url, next_token))
...
def nearly_finished(event):
    ...
    if title is None:
        ...
        if playlist['i'] != 'ShuffleMyPlaylists':
            return do_nothing()
        videos, playlist_title, playlist['sr'] = my_playlists_search(playlist['query'], int(playlist['sr']),
                                                                     playlist['s'])
        for i, id in enumerate(videos):
            playlist['v' + str(i)] = id
            if next_url is None:
...
def play_more_like_this(event):
    ...
    speech_output = strings['playing'] + ' ' + title
    return build_response(
        build_cardless_audio_speechlet_response(speech_output, should_end_session, next_url, next_token))
...
def resume(event, offsetInMilliseconds=None):
    ...
    if title is None:
        ...
        return build_response(build_short_speechlet_response(speech_output, should_end_session))
    return build_response(
        build_cardless_audio_speechlet_response(speech_output, should_end_session, next_url, current_token,
...
def convert_dict_to_token(playlist):
    token = "&".join(["=".join([key, str(val)]) for key, val in list(playlist.items())])
    return token
...
def get_next_url_and_token(current_token, skip):
    ...
    number_of_videos = sum('v' in i for i in list(playlist.keys()))
...
def failed(event):
    ...
    if title is None:
        return do_nothing()
    return build_response(
        build_audio_enqueue_response(should_end_session, next_url, current_token, next_token, playBehavior))
